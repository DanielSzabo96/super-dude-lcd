//Daniel Szabo
//Super Dude LCD Game With Working Sound
...
//initialize inturrupt enables
//irq1 is when user pushes the joystick button
//irq2 is when user hits a wall
...
int melody[] = {
    NOTE_B6,8 , NOTE_B6,8, NOTE_B6,32, REST, 16, REST,32 ,NOTE_A6,32,
    NOTE_B6,32, NOTE_B6,32,REST,32, NOTE_D6,16, NOTE_B6,16, REST, 32, NOTE_A6,16,
    NOTE_B6,8, REST,16, NOTE_G6,8 ,NOTE_G6,32, REST,32, NOTE_FS6,8,
    NOTE_FS6,8, NOTE_FS6,32, REST,32, NOTE_A6,8, NOTE_AS6,8, 
};
...
void playTheme(){
  ...
  for (int thisNote = 0; thisNote < notes * 2; thisNote = thisNote + 2) {
    ...
    tone(MUSIC_BOX, melody[thisNote], noteDuration * 0.9);
...
void playTune(int action){
  if(action == 0) //initialazation sound
  {
    playTheme();
  }

  else if(action == 1) //player jumped
  {
    tone(MUSIC_BOX, NOTE_C6, 100);
  }
  else if(action == 2) //player failed
  {
    tone(MUSIC_BOX, NOTE_C6, 200);
    delay(200);
    tone(MUSIC_BOX, NOTE_G5, 500);
    delay(200);
    tone(MUSIC_BOX, NOTE_B4, 1000);
  }
}
...
void buttonPush() {
  buttonPushed = true;
  if (irq1Enable == 1){ //Prevents playing sound before the game is initialized
    playTune(1);
  }
}

void playerFailed(){
    if (irq2Enable == 1){ //Prevents playing sound before the game is initialized
        playTune(2);
    }
}
